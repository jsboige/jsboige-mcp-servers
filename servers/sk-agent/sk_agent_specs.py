...
(model enabled, MCP known, extends known, agent refs resolvable) run at
resolution time and raise SpecError with actionable messages listing the
valid values.
...
class ConversationSpec(BaseModel):
    ...
    extends: str = ""
    description: str = ""
    type: ConversationTypeLiteral | None = None
...
def resolve_agent_spec(
    ...
    Raises SpecError on unknown references (model/mcp), which the caller
