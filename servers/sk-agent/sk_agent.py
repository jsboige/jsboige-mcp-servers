...
def build_call_agent_description(config: SKAgentConfig) -> str:
    ...
    lines.extend(
        [
            ...
            "  agent_spec: JSON string composing an agent à la carte (#3407): {\"extends\":\"<agent_id>\",\"model\":\"<model_id>\",\"system_prompt\":\"...\",\"mcps\":{\"replace\":[]|\"add\":[],\"remove\":[]},\"memory\":{\"enabled\":false},\"sampling\":{\"temperature\":0..2,\"top_p\":0..1,\"max_tokens\":1..32768}} — precedence: call override > spec > preset > default",
...
class SKAgentManager:
    ...
    def _resolve_effective_mcp_ids(
        ...
        """
        return apply_mcp_delta(base_mcps, mcp_overrides)
    ...
    async def _build_ephemeral_agent(
        ...
        (_collect_mcp_plugins) and the plugin assembly of _create_agent —
        without registering the agent in the persistent per-id maps.
        ...
        agent = ChatCompletionAgent(
            ...
            instructions=agent_cfg.system_prompt or self.config.system_prompt,
            plugins=plugins,
        )
    ...
    async def call_agent(
        ...
        refused. When a spec is given, it consumes model_override /
