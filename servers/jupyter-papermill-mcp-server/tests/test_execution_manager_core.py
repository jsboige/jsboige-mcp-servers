...
class TestJobManagement:
    ...
    def test_start_notebook_async_success(
        ...
        with patch("builtins.open", create=True) as mock_open:
            ...
            result = manager.start_notebook_async(
                input_path="/mock/notebook.ipynb", wait_seconds=0
            )
...
class TestJobStates:
    ...
    @patch("subprocess.Popen")
    @patch("papermill_mcp.services.notebook_service.Path")
    def test_job_completion_success(
        self, mock_path_class, mock_popen, sample_notebook_simple
    ):
...
class TestConcurrencyLimits:
    ...
    @patch("subprocess.Popen")
    @patch("papermill_mcp.services.notebook_service.Path")
    def test_max_concurrent_jobs_enforcement(
        self, mock_path_class, mock_popen, sample_notebook_simple
    ):
        ...
        with patch("builtins.open", create=True) as mock_open:
            ...
            result3 = manager.start_notebook_async(
                "/mock/notebook3.ipynb", wait_seconds=0
            )
    ...
    def test_concurrent_job_creation(
        ...
        def create_job(index):
            with patch("builtins.open", create=True) as mock_open:
                ...
                result = manager.start_notebook_async(
                    f"/mock/notebook{index}.ipynb", wait_seconds=0
                )
