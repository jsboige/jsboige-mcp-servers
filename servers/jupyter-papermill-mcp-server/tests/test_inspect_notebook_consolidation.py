...
class TestInspectNotebookConsolidated:
    ...
    @pytest.mark.asyncio
    async def test_get_notebook_metadata_wrapper_deprecated(
        self, service, basic_notebook
    ):
    ...
    @pytest.mark.asyncio
    async def test_inspect_notebook_outputs_wrapper_deprecated(
        self, service, notebook_with_outputs
    ):
    ...
    @pytest.mark.asyncio
    async def test_inspect_notebook_with_errors_in_outputs(
        self, service, notebook_with_outputs
    ):
    ...
    @pytest.mark.asyncio
    async def test_inspect_notebook_invalid_notebook_structure(
        self, service, invalid_notebook
    ):
    ...
    @pytest.mark.asyncio
    async def test_inspect_notebook_outputs_with_errors(
        self, service, notebook_with_outputs
    ):
    ...
    @pytest.mark.asyncio
    async def test_inspect_notebook_outputs_mixed_types(
        self, service, notebook_with_outputs
    ):
    ...
    async def test_inspect_notebook_file_not_found(self, service):
        ...
        with pytest.raises(FileNotFoundError):
            await service.inspect_notebook(
                "nonexistent_notebook.ipynb", mode="metadata"
            )
...
class TestInspectNotebookIntegration:
    ...
    async def test_inspect_notebook_full_workflow(self, service, notebook_with_outputs):
        ...
        meta_result = await service.inspect_notebook(
            notebook_with_outputs, mode="metadata"
        )
        assert meta_result["success"] is True
        assert "metadata" in meta_result

        # 2. Analyser les outputs
        output_result = await service.inspect_notebook(
            notebook_with_outputs, mode="outputs"
        )
        assert output_result["success"] is True
        assert "output_analysis" in output_result

        # 3. Valider
        valid_result = await service.inspect_notebook(
            notebook_with_outputs, mode="validate"
        )
        ...
        assert (
            full_result["metadata"]["kernelspec"]
            == meta_result["metadata"]["kernelspec"]
        )
        assert (
            full_result["output_analysis"]["total_cells"]
            == output_result["output_analysis"]["total_cells"]
        )
        assert (
            full_result["validation"]["is_valid"]
            == valid_result["validation"]["is_valid"]
        )
