...
class TestExecuteOnKernelModes:
    ...
    async def test_execute_on_kernel_code_mode(self, service, mock_kernel_id):
        ...
        with patch.object(
            service.jupyter_manager, "_active_kernels", {mock_kernel_id: Mock()}
        ):
            with patch.object(
                service,
                "execute_cell",
                return_value={
                    ...
                    "status": "ok",
                    "outputs": [
                        {"output_type": "stream", "content": {"text": "Hello World\n"}}
                    ],
                    "success": True,
                },
            ) as mock_exec:
                result = await service.execute_on_kernel_consolidated(
                    kernel_id=mock_kernel_id, mode="code", code="print('Hello World')"
                )
    ...
    @pytest.mark.asyncio
    async def test_execute_on_kernel_notebook_mode(
        self, service, mock_kernel_id, temp_notebook
    ):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                result = await service.execute_on_kernel_consolidated(
                    kernel_id=mock_kernel_id, mode="notebook", path=str(temp_notebook)
                )
    ...
    @pytest.mark.asyncio
    async def test_execute_on_kernel_notebook_cell_mode(
        self, service, mock_kernel_id, temp_notebook
    ):
...
class TestBackwardCompatibility:
    ...
    async def test_execute_cell_wrapper_deprecated(self, service, mock_kernel_id):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                result = await service.execute_on_kernel_consolidated(
                    kernel_id=mock_kernel_id, mode="code", code="x = 1"
                )
                assert result["mode"] == "code"

    @pytest.mark.asyncio
    async def test_execute_notebook_wrapper_deprecated(
        self, service, mock_kernel_id, temp_notebook
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_notebook_cell_wrapper_deprecated(
        self, service, mock_kernel_id, temp_notebook
    ):
...
class TestExecutionEdgeCases:
    ...
    async def test_execute_on_kernel_code_with_error(self, service, mock_kernel_id):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                result = await service.execute_on_kernel_consolidated(
                    kernel_id=mock_kernel_id, mode="code", code="print(undefined)"
                )
    ...
    @pytest.mark.asyncio
    async def test_execute_on_kernel_notebook_with_errors(
        self, service, mock_kernel_id, temp_notebook
    ):
    ...
    async def test_execute_on_kernel_invalid_kernel_id(self, service):
        ...
        with pytest.raises(ValueError, match="Kernel .* not found"):
            await service.execute_on_kernel_consolidated(
                kernel_id="nonexistent-kernel", mode="code", code="print('test')"
            )

    @pytest.mark.asyncio
    async def test_execute_on_kernel_invalid_cell_index(
        self, service, mock_kernel_id, temp_notebook
    ):
        ...
        with patch.object(
            ...
            with patch.object(
                service,
                "execute_notebook_cell",
                side_effect=IndexError("Cell index out of range"),
            ):
...
class TestParameterValidation:
    ...
    async def test_execute_on_kernel_code_requires_code(self, service, mock_kernel_id):
        ...
        with patch.object(
            ...
            with pytest.raises(
                ValueError, match="Parameter 'code' is required for mode='code'"
            ):
                await service.execute_on_kernel_consolidated(
                    kernel_id=mock_kernel_id,
                    mode="code"
    ...
    @pytest.mark.asyncio
    async def test_execute_on_kernel_notebook_requires_path(
        self, service, mock_kernel_id
    ):
        ...
        with patch.object(
            ...
            with pytest.raises(
                ValueError, match="Parameter 'path' is required for mode='notebook'"
            ):
                await service.execute_on_kernel_consolidated(
                    kernel_id=mock_kernel_id,
                    mode="notebook"
    ...
    @pytest.mark.asyncio
    async def test_execute_on_kernel_notebook_cell_requires_path_and_index(
        self, service, mock_kernel_id
    ):
        ...
        with patch.object(
            ...
            # Test sans path
            with pytest.raises(
                ValueError, match="Parameters 'path' and 'cell_index' are required"
            ):
                await service.execute_on_kernel_consolidated(
                    kernel_id=mock_kernel_id,
                    mode="notebook_cell",
    ...
    async def test_execute_on_kernel_invalid_mode(self, service, mock_kernel_id):
        ...
        with patch.object(
            ...
            with pytest.raises(ValueError, match="Invalid mode"):
                await service.execute_on_kernel_consolidated(
                    kernel_id=mock_kernel_id, mode="invalid_mode", code="print('test')"
                )
...
class TestAsyncExecution:
    ...
    @pytest.mark.asyncio
    async def test_execute_on_kernel_concurrent_executions(
        self, service, mock_kernel_id
    ):
            ...
            with patch.object(
                ...
                tasks = [
                    service.execute_on_kernel_consolidated(
                        kernel_id=mock_kernel_id, mode="code", code=f"x = {i}"
                    )
                    for i in range(3)
                ]
    ...
    async def test_execute_on_kernel_empty_code(self, service, mock_kernel_id):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                result = await service.execute_on_kernel_consolidated(
                    kernel_id=mock_kernel_id, mode="code", code=""
                )
...
"""
        with patch.object(
            ...
            with patch.object(
                ...
                return_value={
                    ...
                    "status": "ok",
                    "outputs": [
                        {"output_type": "stream", "content": {"text": "120\n"}}
                    ],
                    "success": True,
                },
            ):
                result = await service.execute_on_kernel_consolidated(
                    kernel_id=mock_kernel_id, mode="code", code=multiline_code
                )
    ...
    @pytest.mark.asyncio
    async def test_execute_on_kernel_notebook_cell_zero_index(
        self, service, mock_kernel_id, temp_notebook
    ):
