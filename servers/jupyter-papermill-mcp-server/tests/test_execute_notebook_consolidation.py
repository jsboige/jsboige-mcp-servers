...
from papermill_mcp.services.notebook_service_consolidated import (
...
async def test_validate_input_path_not_exists(consolidated_executor, notebook_service):
    ...
    with patch("pathlib.Path.exists", return_value=False):
        result = await consolidated_executor.execute_notebook(
            input_path="nonexistent.ipynb", mode="sync"
        )
...
async def test_validate_parameters_must_be_dict(consolidated_executor):
    ...
    with patch("pathlib.Path.exists", return_value=True):
        result = await consolidated_executor.execute_notebook(
            input_path="test.ipynb", parameters="invalid", mode="sync"  # Should be dict
        )
...
async def test_validate_invalid_mode(consolidated_executor):
    ...
    with patch("pathlib.Path.exists", return_value=True):
        result = await consolidated_executor.execute_notebook(
            input_path="test.ipynb", mode="invalid_mode"
        )
...
async def test_validate_progress_bar_incompatible_with_async(consolidated_executor):
    ...
    with patch("pathlib.Path.exists", return_value=True):
        result = await consolidated_executor.execute_notebook(
            input_path="test.ipynb", mode="async", progress_bar=True
        )
...
async def test_validate_timeout_must_be_positive(consolidated_executor):
    ...
    with patch("pathlib.Path.exists", return_value=True):
        result = await consolidated_executor.execute_notebook(
            input_path="test.ipynb", mode="sync", timeout=-10
        )
...
async def test_validate_invalid_report_mode(consolidated_executor):
    ...
    with patch("pathlib.Path.exists", return_value=True):
        result = await consolidated_executor.execute_notebook(
            input_path="test.ipynb", mode="sync", report_mode="invalid"
        )
...
async def test_execute_notebook_sync_basic(consolidated_executor, notebook_service):
    ...
    with patch("pathlib.Path.exists", return_value=True), patch.object(
        consolidated_executor,
        "_analyze_notebook_output",
        return_value={"cells_executed": 5, "cells_succeeded": 5, "cells_failed": 0},
    ), patch.object(
...
async def test_execute_notebook_sync_with_parameters(
    ...
    with patch("pathlib.Path.exists", return_value=True), patch.object(
        consolidated_executor,
        "_analyze_notebook_output",
        return_value={"cells_executed": 3, "cells_succeeded": 3, "cells_failed": 0},
    ), patch.object(
        consolidated_executor, "_format_report", return_value={"mode": "summary"}
    ):
        result = await consolidated_executor.execute_notebook(
            input_path="test.ipynb", parameters=test_params, mode="sync"
        )
...
@pytest.mark.asyncio
async def test_execute_notebook_sync_custom_output_path(
    consolidated_executor, notebook_service
):
    ...
    with patch("pathlib.Path.exists", return_value=True), patch.object(
        consolidated_executor,
        "_analyze_notebook_output",
        return_value={"cells_executed": 2, "cells_succeeded": 2, "cells_failed": 0},
    ), patch.object(
        consolidated_executor, "_format_report", return_value={"mode": "summary"}
    ):
        result = await consolidated_executor.execute_notebook(
            input_path="test.ipynb", output_path=custom_output, mode="sync"
        )
...
@pytest.mark.asyncio
async def test_execute_notebook_sync_with_error(
    consolidated_executor, notebook_service
):
...
@pytest.mark.asyncio
async def test_execute_notebook_sync_with_timeout(
    consolidated_executor, notebook_service
):
    ...
    with patch("pathlib.Path.exists", return_value=True), patch.object(
        consolidated_executor,
        "_analyze_notebook_output",
        return_value={"cells_executed": 1, "cells_succeeded": 1, "cells_failed": 0},
    ), patch.object(
        consolidated_executor, "_format_report", return_value={"mode": "summary"}
    ):
        result = await consolidated_executor.execute_notebook(
...
async def test_execute_notebook_async_basic(consolidated_executor, notebook_service):
    ...
    with patch("pathlib.Path.exists", return_value=True), patch.object(
        ...
        result = await consolidated_executor.execute_notebook(
            input_path="test.ipynb", mode="async"
        )
...
async def test_execute_notebook_async_with_parameters(
    ...
    with patch("pathlib.Path.exists", return_value=True), patch.object(
        ...
        result = await consolidated_executor.execute_notebook(
            input_path="test.ipynb", parameters=test_params, mode="async"
        )
...
@pytest.mark.asyncio
async def test_execute_notebook_report_mode_minimal(
    consolidated_executor, notebook_service
):
...
@pytest.mark.asyncio
async def test_execute_notebook_report_mode_summary(
    consolidated_executor, notebook_service
):
    ...
    with patch("pathlib.Path.exists", return_value=True), patch.object(
        consolidated_executor,
        "_analyze_notebook_output",
        return_value={"cells_executed": 5, "cells_succeeded": 4, "cells_failed": 1},
    ), patch.object(consolidated_executor, "_format_report") as mock_format:
        ...
        result = await consolidated_executor.execute_notebook(
            input_path="test.ipynb", mode="sync", report_mode="summary"
        )
...
@pytest.mark.asyncio
async def test_execute_notebook_report_mode_full(
    consolidated_executor, notebook_service
):
    ...
    with patch("pathlib.Path.exists", return_value=True), patch.object(
        ...
        result = await consolidated_executor.execute_notebook(
            input_path="test.ipynb", mode="sync", report_mode="full"
        )
...
@pytest.mark.asyncio
async def test_execute_notebook_auto_output_path(
    consolidated_executor, notebook_service
):
    ...
    with patch("pathlib.Path.exists", return_value=True), patch.object(
        ...
        return_value="/auto/output.ipynb",
...
async def test_execute_notebook_papermill_wrapper_deprecated(notebook_service):
    ...
    with patch(
        ...
        notebook_service.execute_notebook_consolidated = AsyncMock(
...
async def test_parameterize_notebook_wrapper_deprecated(notebook_service):
    ...
    with patch("pathlib.Path.exists", return_value=True):
        ...
        result = await notebook_service.execute_notebook_consolidated(
            input_path="test.ipynb", parameters={"param": "value"}, mode="sync"
        )
...
@pytest.mark.asyncio
async def test_execute_notebook_kernel_not_found(
    consolidated_executor, notebook_service
):
...
@pytest.mark.asyncio
async def test_execute_notebook_parameters_types(
    consolidated_executor, notebook_service
):
    ...
    with patch("pathlib.Path.exists", return_value=True), patch.object(
        ...
        result = await consolidated_executor.execute_notebook(
            input_path="test.ipynb", parameters=complex_params, mode="sync"
        )
...
def test_analyze_notebook_output_success():
    ...
    with patch("builtins.open", new_callable=mock_open, read_data="{}"), patch(
...
def test_format_report_minimal():
    ...
    executor = ExecuteNotebookConsolidated(Mock())
    analysis = {"cells_executed": 5, "cells_succeeded": 4, "cells_failed": 1}
...
def test_format_report_summary():
    ...
    executor = ExecuteNotebookConsolidated(Mock())
    analysis = {"cells_executed": 10, "cells_succeeded": 9, "cells_failed": 1}
