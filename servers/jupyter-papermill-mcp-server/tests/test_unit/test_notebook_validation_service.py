...
class TestNotebookValidationService:
    ...
    async def test_inspect_metadata(self, validation_service, valid_notebook_path):
        result = await validation_service.inspect_notebook(
            valid_notebook_path, mode="metadata"
        )
    ...
    async def test_inspect_outputs(self, validation_service, valid_notebook_path):
        result = await validation_service.inspect_notebook(
            valid_notebook_path, mode="outputs"
        )
    ...
    @pytest.mark.asyncio
    async def test_inspect_outputs_with_error(
        self, validation_service, error_notebook_path
    ):
        result = await validation_service.inspect_notebook(
            error_notebook_path, mode="outputs"
        )
    ...
    @pytest.mark.asyncio
    async def test_inspect_validate_valid(
        self, validation_service, valid_notebook_path
    ):
        result = await validation_service.inspect_notebook(
            valid_notebook_path, mode="validate"
        )
    ...
    @pytest.mark.asyncio
    async def test_inspect_validate_invalid(
        self, validation_service, invalid_notebook_path
    ):
        result = await validation_service.inspect_notebook(
            invalid_notebook_path, mode="validate"
        )
    ...
    async def test_inspect_error_handling(self, validation_service, temp_dir):
        ...
        with patch(
            ...
            with pytest.raises(ValueError):
                await validation_service.inspect_notebook(
                    "bad_json.ipynb", mode="metadata"
                )
