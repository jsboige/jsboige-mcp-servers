...
class TestPapermillExecutor:
    ...
    def test_get_available_kernels_success(self, mock_subprocess):
        ...
        with patch(
            "papermill_mcp.core.papermill_executor.get_config"
        ) as mock_get_config:
            mock_papermill = Mock()
    ...
    @patch(
        ...
        read_data='{"metadata": {"kernelspec": {"name": "python3"}}}',
    )
    ...
    def test_generate_output_path(self):
        ...
        with patch(
            ...
            with patch(
                "papermill_mcp.core.papermill_executor.datetime"
            ) as mock_datetime:
                mock_datetime.now.return_value.strftime.return_value = "20231201_120000"
                output_path = executor._generate_output_path(
                    "/input/test.ipynb", "-executed"
                )
...
class TestSingletonFunctions:
    ...
    def test_close_papermill_executor(self):
        ...
        with patch(
            "papermill_mcp.core.papermill_executor.PapermillExecutor"
...
def executor(mock_config):
    ...
    with patch(
        "papermill_mcp.core.papermill_executor.get_config", return_value=mock_config
    ):
...
class TestIntegrationScenarios:
    ...
    @patch("os.chdir")
    @patch("os.getcwd")
    async def test_successful_execution_flow(
        self, mock_getcwd, mock_chdir, mock_exists, mock_pm_execute, executor
    ):
        ...
        with patch(
            "builtins.open",
            mock_open(read_data='{"metadata": {"kernelspec": {"name": "python3"}}}'),
        ):
            # Execution
            result = await executor.execute_notebook(
                input_path="/test/input.ipynb", parameters={"param1": "value1"}
            )
