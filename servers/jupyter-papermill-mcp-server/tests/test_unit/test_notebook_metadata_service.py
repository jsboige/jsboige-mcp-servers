...
class TestNotebookMetadataService:
    ...
    async def test_list_notebooks(self, metadata_service):
        with patch(
            "papermill_mcp.utils.file_utils.FileUtils.list_notebooks"
        ) as mock_list:
    ...
    @pytest.mark.asyncio
    async def test_get_notebook_metadata_legacy(
        self, metadata_service, validation_service
    ):
        ...
        validation_service.inspect_notebook.assert_called_with(
            "path/nb.ipynb", mode="metadata"
        )
