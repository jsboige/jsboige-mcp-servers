...
class TestKernelServiceRefactored:
    ...
    async def test_list_kernels_success(self, kernel_service):
        ...
        kernel_service.jupyter_manager.list_available_kernels.return_value = {
            "python3": {}
        }
    ...
    async def test_execute_cell(self, kernel_service):
        ...
        result = await kernel_service.execute_cell(
            kernel_id="kernel-123", code="print('hi')"
        )

        assert result["status"] == "ok"
        assert len(result["outputs"]) == 1
        assert result["success"] is True
        kernel_service.jupyter_manager.execute_code.assert_called_with(
            "kernel-123", "print('hi')", 60.0
        )
    ...
    async def test_manage_kernel_consolidated(self, kernel_service):
        ...
        # Test start
        res_start = await kernel_service.manage_kernel_consolidated(
            action="start", kernel_name="py3"
        )
        assert res_start["status"] == "started"
        kernel_service.jupyter_manager.start_kernel.assert_called_with("py3")

        # Test stop
        res_stop = await kernel_service.manage_kernel_consolidated(
            action="stop", kernel_id="123"
        )
        ...
        with pytest.raises(ValueError):
            await kernel_service.manage_kernel_consolidated(
                action="invalid", kernel_id="123"
            )
    ...
    async def test_execute_on_kernel_consolidated(self, kernel_service):
        ...
        res = await kernel_service.execute_on_kernel_consolidated(
            kernel_id="123", mode="code", code="print(1)"
        )
        ...
        # Test invalid mode
        with pytest.raises(ValueError):
            await kernel_service.execute_on_kernel_consolidated(
                kernel_id="123", mode="invalid"
            )
