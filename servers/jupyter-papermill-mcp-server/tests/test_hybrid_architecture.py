...
class TestSyncAsyncSwitching:
    ...
    def test_sync_execution_quick_notebook(
    ...
    @patch("subprocess.Popen")
    @patch("papermill_mcp.services.notebook_service.Path")
    def test_async_execution_complex_notebook(
...
class TestJobPolling:
    ...
    def test_job_status_polling(
        ...
        with patch("builtins.open", create=True) as mock_open:
            ...
            # Start job
            result = manager.start_notebook_async(
                "/mock/notebook.ipynb", wait_seconds=0
            )
...
class TestHybridArchitectureIntegration:
    ...
    @patch("subprocess.Popen")
    @patch("papermill_mcp.services.notebook_service.Path")
    def test_multiple_notebooks_different_complexities(
        self,
        mock_path_class,
        mock_popen,
        sample_notebook_simple,
        ...
        with patch("builtins.open", create=True) as mock_open:
            ...
            result_simple = manager.start_notebook_async(
                "/mock/simple_notebook.ipynb", wait_seconds=0
            )
            result_complex = manager.start_notebook_async(
                "/mock/complex_notebook.ipynb", wait_seconds=0
            )
