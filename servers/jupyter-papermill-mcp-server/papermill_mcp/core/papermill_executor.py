...
class PapermillExecutor:
    ...
    def _get_available_kernels(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            kernel_timeout = getattr(
                self.config.papermill, "kernel_detection_timeout", 60
            )
        ...
        try:
            ...
            if preferred_kernel:
                ...
                if preferred_kernel in available_kernels:
                    self.logger.info(
                        f"Kernel detected from metadata: {preferred_kernel}"
                    )
                    return preferred_kernel
                else:
                    self.logger.warning(
                        f"Preferred kernel '{preferred_kernel}' not available"
                    )
            ...
            if (
                ".net" in nb_content or "csharp" in nb_content
            ) and ".net-csharp" in available_kernels:
                return ".net-csharp"
    ...
    async def execute_notebook(
        ...
        if kernel is None:
            ...
            if kernel is None:
                ...
                error_msg = (
                    f"No suitable kernel found. Available kernels: {available_kernels}"
                )
                self.logger.error(error_msg)
                return ExecutionResult(
                    success=False,
        ...
        if timeout is None:
            ...
            if "semantickernel" in notebook_name:
                ...
                if "04" in notebook_name or "clr" in notebook_name:
                    timeout = max(
                        timeout, 1200
                    )  # 20 minutes pour notebook 04 (pythonnet installation)
                    self.logger.info(
                        f"🔧 Extended timeout for .NET CLR notebook: {timeout}s"
                    )
                elif "05" in notebook_name or "notebookmaker" in notebook_name:
                    timeout = max(
                        timeout, 900
                    )  # 15 minutes pour notebook 05 (widgets + API)
                    self.logger.info(
                        f"🎛️ Extended timeout for widgets notebook: {timeout}s"
                    )
                else:
                    timeout = max(
                        timeout, 720
                    )  # 12 minutes pour autres notebooks SemanticKernel
                    self.logger.info(
                        f"🧠 Extended timeout for SemanticKernel notebook: {timeout}s"
                    )
        ...
        try:
            ...
            def safe_working_directory(target_dir):
                ...
                try:
                    ...
                    try:
                        ...
                        self.logger.debug(
                            f"🔄 Restored working directory: {original_cwd}"
                        )
                    except Exception as restore_error:
                        self.logger.error(
                            f"⚠️ CRITICAL: Failed to restore working directory: {restore_error}"
                        )

            with safe_working_directory(notebook_dir):
                # Optimisation : Injection .NET seulement si nécessaire ou si kernel inconnu
                is_dotnet_kernel = kernel and (
                    ".net" in kernel.lower()
                    or "csharp" in kernel.lower()
                    or "fsharp" in kernel.lower()
                    or "powershell" in kernel.lower()
                )

                if is_dotnet_kernel:
                    self.logger.info(
                        f"🚀 .NET Kernel detected ({kernel}): Injecting .NET environment..."
                    )
                    ...
                    with inject_dotnet_environment() as injected_vars:
                        if injected_vars:
                            self.logger.info(
                                f".NET environment injected: {len(injected_vars)} variables"
                            )
                            ...
                            for var in critical_vars:
                                if var in injected_vars:
                                    self.logger.debug(
                                        f"  [OK] {var}={injected_vars[var]}"
                                    )
                        else:
                            self.logger.warning(
                                "WARNING: No .NET environment variables injected"
                            )
                    ...
                    self.logger.info(
                        f"🐍 Python/Other Kernel detected ({kernel}): Skipping .NET injection optimization"
                    )
            ...
            if metrics.execution_time_seconds > 0:
                metrics.cells_per_second = (
                    metrics.total_cells / metrics.execution_time_seconds
                )
            ...
            self.logger.info(
                f"Execution successful in {metrics.execution_time_seconds:.2f}s"
            )
    ...
    def _extract_error_context(self, error: PapermillExecutionError) -> List[str]:
        ...
        if "modulenotfounderror" in error_str:
            context.append(
                "Suggestion: Check that required Python packages are installed"
            )
