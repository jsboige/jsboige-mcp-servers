...
class NotebookService:
    ...
    async def add_cell(
        ...
        Returns:
            Dictionary with operation result
        """
        return await self.crud_service.add_cell(
            path, cell_type, source, metadata, index
        )
    ...
    async def read_cells_range(
        self, path: Union[str, Path], start_index: int, end_index: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        return await self.read_cells(
            path, mode="range", start_index=start_index, end_index=end_index
        )
    ...
    async def inspect_notebook_outputs(self, path: Union[str, Path]) -> Dict[str, Any]:
        ...
        """
        logger.warning(
            "inspect_notebook_outputs is deprecated, use inspect_notebook(mode='outputs') instead"
        )
    ...
    async def validate_notebook(self, path: Union[str, Path]) -> Dict[str, Any]:
        ...
        for error in validation.get("errors", []):
            if error.get("cell_index") is not None:
                ...
                existing = next(
                    (c for c in cell_issues if c["cell_index"] == cell_idx), None
                )
                if existing:
                    existing["issues"].append(error["message"])
                else:
                    cell_issues.append(
                        {"cell_index": cell_idx, "issues": [error["message"]]}
                    )
    ...
    async def inspect_notebook(
        self, path: Union[str, Path], mode: str = "metadata"
    ) -> Dict[str, Any]:
        """
    ...
    async def list_kernels(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"kernels": specs, "default": ksm.default_kernel_name}
        except Exception as e:
