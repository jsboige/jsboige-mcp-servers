...
class KernelService:
    ...
    async def list_kernels(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Found {len(available_kernels)} available kernels, {len(active_kernels)} active"
            )
            return result
    ...
    async def stop_kernel(self, kernel_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {"kernel_id": kernel_id, "status": "stopped", "success": True}
    ...
    async def interrupt_kernel(self, kernel_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {"kernel_id": kernel_id, "status": "interrupted", "success": True}
    ...
    async def execute_cell(
        ...
        try:
            ...
            logger.debug(
                f"Code to execute: {code[:100]}{'...' if len(code) > 100 else ''}"
            )
            ...
            for output in result.outputs:
                if hasattr(output, "__dict__"):
                    ...
                    if (
                        hasattr(output, "execution_count")
            ...
            execution_dict = {
                ...
                "error": result.error_name or result.error_value
                if (result.error_name or result.error_value)
                else None,
    ...
    async def execute_notebook_in_kernel(
        ...
        try:
                ...
                if cell.cell_type == "code" and cell.source.strip():
                    ...
                    try:
                        ...
                        cell_dict = {
                            ...
                            "error": cell_result.error_name or cell_result.error_value
                            if (cell_result.error_name or cell_result.error_value)
                            else None,
                        ...
                        if (
                            ...
                            logger.warning(
                                f"Stopping execution due to error in cell {i}"
                            )
                            break
            ...
            logger.error(
                f"Error executing notebook {notebook_path} in kernel {kernel_id}: {e}"
            )
            raise
    ...
    async def execute_notebook_cell(
        ...
        try:
            logger.info(
                f"Executing cell {cell_index} from notebook {notebook_path} in kernel {kernel_id}"
            )
            ...
            if cell_index < 0 or cell_index >= len(notebook.cells):
                raise IndexError(
                    f"Cell index {cell_index} out of range (0-{len(notebook.cells)-1})"
                )
            ...
            logger.info(
                f"Cell {cell_index} execution completed with status: {cell_result.status}"
            )
            return result

        except Exception as e:
            logger.error(
                f"Error executing cell {cell_index} from notebook {notebook_path}: {e}"
            )
            raise
    ...
    async def cleanup_kernels(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for kernel in active_kernels:
                ...
                try:
                    ...
                    cleanup_results.append(
                        {"kernel_id": kernel_id, "status": "stopped", "success": True}
                    )
            ...
            logger.info(
                f"Cleanup completed: {successful_cleanups}/{len(active_kernels)} kernels stopped"
            )
            return result
    ...
    async def manage_kernel_consolidated(
        ...
        """
        Consolidated method for managing kernel lifecycle.
        """
        if action == "start":
            if not kernel_name:
                raise ValueError("kernel_name is required for start action")
            # working_dir is not currently supported by start_kernel but could be added
            return await self.start_kernel(kernel_name)

        if not kernel_id:
            raise ValueError(f"kernel_id is required for {action} action")

        if action == "stop":
            return await self.stop_kernel(kernel_id)
        elif action == "interrupt":
            return await self.interrupt_kernel(kernel_id)
        elif action == "restart":
            return await self.restart_kernel(kernel_id)
        else:
    ...
    async def execute_on_kernel_consolidated(
        ...
        """
        Consolidated method for executing code/notebooks on a kernel.
        """
        if mode == "code":
            if code is None:
                raise ValueError("code is required for mode='code'")
            # Execute cell logic handles single code block
            result = await self.execute_cell(kernel_id, code, float(timeout))

            # Reshape result to match consolidated output format if needed
            # For now returning as-is mostly matches but we wrap it
            return {
                "kernel_id": kernel_id,
                "mode": "code",
