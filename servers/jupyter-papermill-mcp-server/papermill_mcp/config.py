...
class JupyterServerConfig(BaseModel):
    ...
    base_url: str = Field(
        default="http://localhost:8888", description="Jupyter server base URL"
    )
...
class PapermillConfig(BaseModel):
    ...
    output_dir: str = Field(
        default="./outputs", description="Directory for output notebooks"
    )
    timeout: int = Field(
        default=900,
        description="Execution timeout in seconds (15 minutes for complex notebooks)",
    )
    kernel_detection_timeout: int = Field(
...
class ConfigManager:
    ...
    def load_config(self, options: Optional[CommandLineOptions] = None) -> MCPConfig:
        ...
        if config_file_path and Path(config_file_path).exists():
            try:
                ...
                print(
                    f"Erreur lors du chargement du fichier de configuration {config_file_path}: {e}"
                )
                print("Utilisation des valeurs par defaut")
        else:
            if config_file_path:
                print(
                    f"Fichier de configuration {config_file_path} non trouve, utilisation des valeurs par defaut"
                )
    ...
    def _determine_config_file_path(self, options: CommandLineOptions) -> Optional[str]:
        ...
        return options.config or os.getenv("JUPYTER_MCP_CONFIG") or "./config.json"
    ...
    def _apply_environment_variables(self, config_dict: Dict[str, Any]):
        ...
        if os.getenv("JUPYTER_MCP_TIMEOUT"):
            if "papermill" not in config_dict:
                config_dict["papermill"] = {}
            try:
                config_dict["papermill"]["timeout"] = int(
                    os.getenv("JUPYTER_MCP_TIMEOUT")
                )
            except ValueError:
                print(
    ...
    def _apply_command_line_options(
        self, config_dict: Dict[str, Any], options: CommandLineOptions
    ):
