 ...
 * @returns {boolean} - true si les arguments sont valides, false sinon
 */
const isValidExtractMarkdownStructureArgs = (args) => {
    if (typeof args !== 'object' || args === null)
        return false;
    // Vérification du tableau paths
    if (!Array.isArray(args.paths))
    ...
    if (args.max_depth !== undefined && typeof args.max_depth !== 'number')
        return false;
    if (args.include_context !== undefined && typeof args.include_context !== 'boolean')
        return false;
 ...
 * @returns {boolean} - true si les arguments sont valides, false sinon
 */
const isValidCopyFilesArgs = (args) => {
    if (typeof args !== 'object' || args === null)
        return false;
    ...
    for (const operation of args.operations) {
        ...
        if (operation.transform !== undefined) {
            if (typeof operation.transform !== 'object' || operation.transform === null)
                return false;
            if (typeof operation.transform.pattern !== 'string')
                return false;
            if (typeof operation.transform.replacement !== 'string')
                return false;
        }
 ...
 * @returns {boolean} - true si les arguments sont valides, false sinon
 */
const isValidMoveFilesArgs = (args) => {
 ...
 * @returns {boolean} - true si les arguments sont valides, false sinon
 */
const isValidSearchInFilesArgs = (args) => {
    if (typeof args !== 'object' || args === null)
        return false;
    ...
    // Vérification des paramètres optionnels
    if (args.use_regex !== undefined && typeof args.use_regex !== 'boolean')
        return false;
    ...
    if (args.context_lines !== undefined && typeof args.context_lines !== 'number')
        return false;
    if (args.max_results_per_file !== undefined && typeof args.max_results_per_file !== 'number')
        return false;
    if (args.max_total_results !== undefined && typeof args.max_total_results !== 'number')
        return false;
...
const isValidSearchAndReplaceArgs = (args) => {
    ...
    if (args.files !== undefined) {
        ...
        for (const file of args.files) {
            ...
            if (typeof file.path !== 'string')
                return false;
            if (typeof file.search !== 'string')
                return false;
            if (typeof file.replace !== 'string')
                return false;
            // Vérification des paramètres optionnels
            if (file.use_regex !== undefined && typeof file.use_regex !== 'boolean')
                return false;
            if (file.case_sensitive !== undefined && typeof file.case_sensitive !== 'boolean')
                return false;
            if (file.preview !== undefined && typeof file.preview !== 'boolean')
                return false;
        }
    }
    ...
    if (args.recursive !== undefined && typeof args.recursive !== 'boolean')
        return false;
    if (args.preview !== undefined && typeof args.preview !== 'boolean')
        return false;
    return true;
};
/**
...
class QuickFilesServer {
    ...
    formatDirectoryContents(dirPath, contents) {
        ...
        const markdownFiles = contents.filter(item => item.type === 'file' &&
     ...
     * @throws {McpError} - Erreur si les paramètres sont invalides
     */
    async handleExtractMarkdownStructure(request) {
        if (!isValidExtractMarkdownStructureArgs(request.params.arguments)) {
            throw new McpError(ErrorCode.InvalidParams, 'Paramètres invalides pour extract_markdown_structure');
        }
        const { paths, max_depth = 6, include_context = false, context_lines = 2 } = request.params.arguments;
        try {
            const results = await Promise.all(paths.map(async (filePath) => {
                try {
                    ...
                    const { headings, fileInfo } = this.extractMarkdownHeadings(content, filePath, max_depth, include_context ? context_lines : 0);
                    return {
                        path: filePath,
                        exists: true,
                        headings,
                    ...
                    return {
                        ...
                        exists: false,
                        headings: [],
                        error: `Erreur lors de l'analyse du fichier markdown: ${error.message}`,
                    };
                }
            }));
            // Formatage de la réponse pour une meilleure lisibilité
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Erreur lors de l'analyse des fichiers markdown: ${error.message}`,
                    },
                ],
                isError: true,
            };
     ...
     */
    extractMarkdownHeadings(content, filePath, maxDepth = 6, contextLines = 0) {
     ...
     * @throws {McpError} - Erreur si les paramètres sont invalides
     */
    async handleCopyFiles(request) {
        if (!isValidCopyFilesArgs(request.params.arguments)) {
            throw new McpError(ErrorCode.InvalidParams, 'Paramètres invalides pour copy_files');
        }
        ...
        try {
            const results = await Promise.all(operations.map(async (operation) => {
                try {
                    ...
                    const fileResults = await Promise.all(sourcePaths.map(async (sourcePath) => {
                        try {
                            ...
                            return {
                                ...
                                success: false,
                                error: `Erreur lors de la copie: ${error.message}`
                            };
                        }
                    }));
                    ...
                    return {
                        ...
                        success: false,
                        error: `Erreur lors de l'opération: ${error.message}`,
                        files: []
                    };
                }
            }));
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Erreur lors de la copie des fichiers: ${error.message}`,
                    },
                ],
                isError: true,
            };
     ...
     * @throws {McpError} - Erreur si les paramètres sont invalides
     */
    async handleMoveFiles(request) {
        if (!isValidMoveFilesArgs(request.params.arguments)) {
            throw new McpError(ErrorCode.InvalidParams, 'Paramètres invalides pour move_files');
        }
        const { operations } = request.params.arguments;
        try {
            const results = await Promise.all(operations.map(async (operation) => {
                    ...
                    const fileResults = await Promise.all(sourcePaths.map(async (sourcePath) => {
                        try {
                            ...
                            return {
                                ...
                                destination: isDestDir ? path.join(destination, path.basename(sourcePath)) : destination,
                                success: false,
                                error: `Erreur lors du déplacement: ${error.message}`
                            };
                        }
                    }));
                    // Calculer le résultat global pour cette opération
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Erreur lors du déplacement des fichiers: ${error.message}`,
                    },
                ],
                isError: true,
            };
     ...
     * @throws {McpError} - Erreur si les paramètres sont invalides
     */
    async handleSearchInFiles(request) {
        if (!isValidSearchInFilesArgs(request.params.arguments)) {
            throw new McpError(ErrorCode.InvalidParams, 'Paramètres invalides pour search_in_files');
        }
        const { paths, pattern, use_regex = true, case_sensitive = false, file_pattern, context_lines = 2, max_results_per_file = 100, max_total_results = 1000, recursive = true } = request.params.arguments;
        try {
            ...
            if (use_regex) {
                try {
                    ...
                    throw new McpError(ErrorCode.InvalidParams, `Expression régulière invalide: ${error.message}`);
                }
            }
            else {
            ...
            const searchInDirectory = async (dirPath, currentDepth = 1) => {
                ...
                try {
                    ...
                    console.error(`[ERROR] Erreur lors de la recherche dans le répertoire ${dirPath}: ${error.message}`);
                }
            };
            ...
            const searchInFile = async (filePath) => {
                ...
                try {
                    ...
                    console.error(`[ERROR] Erreur lors de la recherche dans le fichier ${filePath}: ${error.message}`);
                }
            };
            ...
            for (const searchPath of paths) {
                ...
                try {
                    ...
                    console.error(`[ERROR] Erreur lors de l'accès au chemin ${searchPath}: ${error.message}`);
                }
            }
            // Formatage de la réponse pour une meilleure lisibilité
     ...
     * @throws {McpError} - Erreur si les paramètres sont invalides
     */
    async handleSearchAndReplace(request) {
        if (!isValidSearchAndReplaceArgs(request.params.arguments)) {
            throw new McpError(ErrorCode.InvalidParams, 'Paramètres invalides pour search_and_replace');
        }
        const { files, paths, search, replace, use_regex = true, case_sensitive = false, file_pattern, recursive = true, preview = false } = request.params.arguments;
        try {
            ...
            const replaceInFile = async (filePath, searchPattern, replacement, useRegex, caseSensitive, isPreview) => {
                try {
                    // Lire le contenu du fichier
                    const content = await fs.readFile(filePath, 'utf-8');
                    // Créer l'expression régulière pour la recherche
                    ...
                    if (useRegex) {
                        try {
                            ...
                            throw new Error(`Expression régulière invalide: ${error.message}`);
                        }
                    }
                    else {
                        // Échapper les caractères spéciaux pour une recherche de texte littéral
                    ...
                    return {
                        path: filePath,
                        error: `Erreur lors du remplacement: ${error.message}`,
                        modified: false,
                        diff: ''
                    };
                }
            ...
            // Fonction récursive pour rechercher dans un répertoire
            const processDirectory = async (dirPath, currentDepth = 1) => {
                try {
                    const entries = await fs.readdir(dirPath, { withFileTypes: true });
                    for (const entry of entries) {
                        const entryPath = path.join(dirPath, entry.name);
                        if (entry.isDirectory()) {
                            ...
                            const result = await replaceInFile(entryPath, search, replace, use_regex, case_sensitive, preview);
                    ...
                    console.error(`[ERROR] Erreur lors du traitement du répertoire ${dirPath}: ${error.message}`);
                }
            };
            // Traiter les fichiers spécifiés individuellement
            if (files && files.length > 0) {
                for (const file of files) {
                    const result = await replaceInFile(file.path, file.search, file.replace, file.use_regex !== undefined ? file.use_regex : use_regex, file.case_sensitive !== undefined ? file.case_sensitive : case_sensitive, file.preview !== undefined ? file.preview : preview);
                    if (result.replacementCount > 0 || result.error) {
                        results.push(result);
                    }
                }
                ...
                for (const searchPath of paths) {
                    try {
                        ...
                        if (stats.isDirectory()) {
                            ...
                            // Traiter le fichier
                            const result = await replaceInFile(searchPath, search, replace, use_regex, case_sensitive, preview);
                            if (result.replacementCount > 0 || result.error) {
                                results.push(result);
                            }
                        }
