...
export const DashboardArgsSchema = z.object({
  ...
  sourceKey: z.string().optional()
    .describe('(merge) Raw source key to merge INTO this dashboard (exact key as listed by action=list, e.g. "machine-myia-po-2025 (1)"). Its intercom journal is unioned into the target (by message id, latest timestamp wins), the freshest status is kept, then the source key is archived and removed from BOTH artifacts (file + PG) — gated on the PG half actually persisting the union. This is the only operation that repairs a fork key in the store; filesystem gestures (cp/mv/rm) are inert on the PG half.'),
...
export const dashboardToolMetadata = {
  ...
  description: 'Shared dashboards (global/machine/workspace). Actions: read, write, append, list, delete, merge, read_archive, read_overview, refresh, update (v3 create-or-replace on the status section, same keys as write; intercom is append-only via append). Team stages supported. For agent-parseable output, use format="json" on read/read_overview actions. Default is human-readable markdown. Gotchas: (1) Only 3 types exist: global, machine, workspace. (2) If response contains "written to file:", use Read tool on that file path. (3) Auto-condensation at 92% — no manual condense needed. (4) merge is the only key-repair operation (fork/twin keys) — pass the raw sourceKey listed by action=list; it writes BOTH artifacts (GDrive file + PG), archives the source via atomic rename, and removes the source only when the PG half verifiably holds the union.',
  inputSchema: (() => {
