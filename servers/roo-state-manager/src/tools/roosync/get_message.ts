...
- Utilisez \`roosync_messages\` avec \`action: "inbox"\` pour lister les messages disponibles`
    ...
    // #1017-bis : avertissement inline si le mark_read auto a échoué (lecture OK, write KO)
    if (markReadWarning) {
    ...
    if (message.status === 'unread') {
      result += `- ✉️ **Marquer comme lu** : Utilisez \`roosync_messages\` avec \`action: "mark_read"\` et \`message_id\`\n`;
    }

    if (message.status !== 'archived') {
      result += `- 📦 **Archiver** : Utilisez \`roosync_messages\` avec \`action: "archive"\` et \`message_id\`\n`;
    }
