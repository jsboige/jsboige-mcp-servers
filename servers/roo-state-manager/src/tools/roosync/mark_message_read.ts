...
Le message était déjà marqué comme lu. Aucune modification nécessaire.`
    ...
    result += `- 📦 **Archiver** : Utilisez \`roosync_messages\` avec \`action: "archive"\` et \`message_id: "${args.message_id}"\` pour archiver ce message\n`;
    result += `- 💬 **Répondre** : Utilisez \`roosync_messages\` avec \`action: "reply"\` et \`message_id: "${args.message_id}"\` pour répondre à ce message\n`;
    result += `- 📋 **Voir détails** : Utilisez \`roosync_messages\` avec \`action: "message"\` et \`message_id: "${args.message_id}"\` pour voir le contenu complet\n`;
