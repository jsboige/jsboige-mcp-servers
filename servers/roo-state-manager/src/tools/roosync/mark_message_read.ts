...
Le message était déjà marqué comme lu. Aucune modification nécessaire.`
    ...
    result += `- 📦 **Archiver** : Utilisez \`roosync_messages\` avec \`action: "archive"\` et \`message_id\` pour archiver ce message\n`;
    result += `- 💬 **Répondre** : Utilisez \`roosync_messages\` avec \`action: "reply"\` et \`message_id\` pour répondre à ce message\n`;
