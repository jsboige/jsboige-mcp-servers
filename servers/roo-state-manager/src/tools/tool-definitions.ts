...
export const roosyncIndexingDefinition = {
    ...
    description: "Manage semantic index, cache, and archiving (index, reset, rebuild, diagnose, archive, status, repair_gaps, repair_workspace, cleanup, garbage_scan, cleanup_orphans, tool_usage_stats, save_snapshot, trend_report)",
    inputSchema: {
        type: 'object',
        properties: {
...
export const roosyncMessagesDefinition = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            subject: { type: 'string' },
            body: { type: 'string' },
            priority: { type: 'string', enum: ['', 'LOW', 'MEDIUM', 'HIGH', 'URGENT'], description: 'Priority (default MEDIUM) — send/reply + bulk filters ONLY. For action=inbox: omit it, or send "" when your binding requires every field; a real priority is rejected as bulk-only (#3351)' },
            tags: { type: 'array', items: { type: 'string' } },
            thread_id: { type: 'string' },
            ...
            before_date: { type: 'string', description: 'Bulk only (bulk_mark_read/bulk_archive) — rejected on inbox (#3351)' },
