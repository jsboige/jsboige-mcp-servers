...
const createMockTask = async (taskId, parentId = null, title = '') => {
    ...
    if (parentId) {
        const parentChildren = await globalCacheManager.get(`children-index:${parentId}`) || [];
...
describe('Navigation Arborescente des Tâches E2E', () => {
    ...
    // │   └── grandchild-1
    // └── child-2
    const setupHierarchy = async (cache) => {
...
//# sourceMappingURL=task-navigation.test.js.map