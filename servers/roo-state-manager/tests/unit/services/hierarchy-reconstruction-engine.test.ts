/**
 * Tests unitaires pour le moteur de reconstruction hi�rarchique
 * Valide le fonctionnement du syst�me en deux passes
 */
...
describe('HierarchyReconstructionEngine', () => {
    ...
    beforeEach(() => {
        ...
        // R�initialiser l'engine pour chaque test
    ...
    describe('Phase 1 - Extraction et indexation', () => {
        it('should extract new_task patterns from XML tags', async () => {
            ...
            if (result.errors.length > 0) {
                console.error('Phase 1 Errors:', JSON.stringify(result.errors, null, 2));
            }
        ...
        it('should extract task patterns from simple tags', async () => {
            ...
            const mockMessages = [
                {
                    role: 'assistant',
                    content: '<task>Impl�menter la fonctionnalit� de recherche</task>',
                    timestamp: Date.now()
                }
            ];
        ...
        it('should extract delegation patterns', async () => {
            ...
            const mockMessages = [
                {
                    role: 'assistant',
                    content: 'Je te passe en mode orchestrator pour g�rer les sous-t�ches',
                    timestamp: Date.now()
                }
            ];
        ...
        it('should deduplicate instructions', async () => {
            ...
            const duplicateMessages = [
                {
                    role: 'assistant',
                    content: '<new_task><mode>code</mode><message>M�me t�che</message></new_task>',
                    timestamp: 1000
                },
                {
                    role: 'assistant',
            ...
            // V�rifier que les doublons sont g�r�s (selon l'impl�mentation)
            expect(result.parsedCount).toBe(1);
            if (skeleton.parsedSubtaskInstructions) {
                // L'impl�mentation actuelle peut garder les doublons avec timestamps diff�rents
        ...
        it('should skip re-parsing when checksums are identical', async () => {
            ...
            mockedFs.existsSync.mockReturnValue(true);
            // Retourner le m�me contenu (donc m�me checksum)
            mockedFs.readFileSync.mockImplementation((filePath: any) => {
        ...
        it('should build radix tree with 200 char prefixes', async () => {
            const skeleton = enhanceSkeleton(mockSkeletons[0]);
            const longInstruction = 'A'.repeat(300); // Plus de 200 caract�res
            ...
            expect(result.parsedCount).toBe(1);
            if (skeleton.parsedSubtaskInstructions) {
                // V�rifier que le message est tronqu� � 200 chars dans l'instruction
    ...
    describe('Phase 2 - R�solution des parentIds', () => {
        it('should find parent by similarity search', async () => {
            ...
            // La similarit� devrait permettre de trouver le parent
        ...
        it('should validate temporal constraints', async () => {
            ...
            // Le parent cr�� APR�S l'enfant ne devrait pas �tre accept�
        ...
        it('should validate workspace isolation', async () => {
            ...
            // L'orphelin du second workspace ne doit pas �tre li� au premier
            skeletons[1].parentTaskId = undefined;

            const result = await engine.executePhase2(skeletons);

            // Ne devrait pas lier des t�ches de workspaces diff�rents
        ...
        it('should use fallback methods (metadata ? temporal)', async () => {
            ...
            skeletons[2].parentTaskId = undefined;
            skeletons[2].truncatedInstruction = 'Texte sans rapport'; // Pas de similarit�

            const result = await engine.executePhase2(skeletons);

            if (result.resolvedCount > 0) {
                expect(result.resolutionMethods).toBeDefined();
                // V�rifier qu'une m�thode de fallback a �t� utilis�e
        ...
        it('should mark root tasks with isRootTask flag', async () => {
            const skeletons = [
                enhanceSkeleton({
                    ...mockSkeletons[0],
                    truncatedInstruction: 'Bonjour, je voudrais cr�er',
        ...
        it('should not modify valid existing parentIds', async () => {
            ...
            // Le parentId valide ne devrait pas �tre modifi�
        ...
        it('should handle invalid parentIds', async () => {
            ...
            // Le parentId invalide devrait �tre r�solu ou marqu� comme non r�solu
        ...
        it('should respect minimum confidence threshold', async () => {
            ...
            const engineWithHighThreshold = new HierarchyReconstructionEngine({
                minConfidenceScore: 0.9 // Seuil tr�s �lev�
            });

            const result = await engineWithHighThreshold.executePhase2(skeletons);

            // Avec un seuil �lev�, moins de r�solutions
            if (result.resolvedCount === 0) {
    ...
    describe('Incremental recovery', () => {
        it('should resume from phase 1 checkpoint', async () => {
            ...
            // Simuler une interruption apr�s Phase 1
            skeleton.processingState.phase1Completed = true;
        ...
        it('should only process missing parentIds', async () => {
            ...
            expect(result.processedCount).toBeGreaterThan(0);
            expect(skeletons[1].reconstructedParentId).toBeUndefined(); // Pas touch�
        });
        ...
        it('should force rebuild when requested', async () => {
            ...
            expect(result.processedCount).toBe(1); // Devrait reprocesser malgr� le cache
        });
    });
    ...
    describe('Error handling and edge cases', () => {
        ...
        it('should handle timeout scenarios', async () => {
            ...
            // Devrait compl�ter sans timeout (car op�rations rapides)
    ...
    describe('Integration with static method', () => {
        it('should reconstruct hierarchy using static method', async () => {
            // Ce test n�cessite une approche diff�rente pour mocker le module
            // Pour simplifier, on teste juste que la m�thode statique existe et fonctionne
            mockedFs.existsSync.mockReturnValue(false);

            // On teste que la m�thode existe et peut �tre appel�e
            ...
            // Le vrai test d'int�gration serait dans un fichier s�par�
    ...
    describe('Performance tests', () => {
        it('should handle 1000+ entries efficiently', async () => {
            ...
            const largeDataset = generateLargeDataset(100).map(enhanceSkeleton); // R�duit pour les tests
            ...
            // Devrait compl�ter en temps raisonnable
