...
| `vitest.config.ci.ts` | CI (GitHub Actions) | + 30 declared test-file exclusions (census: [docs/CI-EXCLUSIONS-CENSUS.md](./docs/CI-EXCLUSIONS-CENSUS.md)) |
...
| Outil | Description |
|-------|-------------|
| `conversation_browser` | Navigate, visualize and summarize conversations (actions: list, tree, current, view, summarize, rebuild) |

### 🔍 Recherche & Indexation (3 outils)
| Outil | Description |
|-------|-------------|
| `roosync_search` | Recherche unifiée dans les tâches Roo (semantic, text, index diagnostics) |
| `roosync_indexing` | Manage semantic index, cache, and archiving (index, reset, rebuild, diagnose, archive, status, repair_gaps, cleanup) |
| `codebase_search` | Recherche sémantique dans le code par concept (workspace requis) |

### 📊 Exports (1 outil)
| Outil | Description |
|-------|-------------|
| `export_data` | Export data as XML, JSON, CSV, or Markdown (targets: task, conversation, project) |

### 🛠️ VSCode & Debug (1 outil)
| Outil | Description |
|-------|-------------|
...
| Outil | Description |
|-------|-------------|
| `roosync_inventory` | Machine inventory, heartbeat status, system snapshot, cluster health (type: status/health/machines) |
| `roosync_baseline` | Baseline management (actions: update, version, restore, export, list_versions, current_version) |

### 🔧 Maintenance (2 outils)
| Outil | Description |
|-------|-------------|
| `roosync_storage_management` | Storage inspection and maintenance (actions: storage, maintenance; subactions: detect, stats, cache_rebuild, diagnose_bom, repair_bom) |
| `roosync_diagnose` | Diagnostics and debug (actions: env, debug, reset, test, health, lifecycle, analyze, best-practices) |

### 💬 Messaging (1 outil)
| Outil | Description |
|-------|-------------|
| `roosync_messages` | Messagerie inter-machines (actions: send, reply, amend, inbox, message, mark_read, archive, bulk_*, cleanup, stats, attachments_*) |
